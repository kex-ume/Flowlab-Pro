(() => {
  ...
  let masterEquipmentRunIndex = null;
  ...
  $('#runTable').addEventListener('click',event=>{const remove=event.target.closest('[data-delete-run]');if(remove){currentPoint().runs.splice(Number(remove.dataset.deleteRun),1);currentPoint().backendResult=null;renderRuns();renderResult();markDraftDirty();return;}const select=event.target.closest('[data-master-equipment]');if(select){masterEquipmentRunIndex=Number(select.dataset.masterEquipment);equipmentSourceIndex=null;$('#equipmentModal').hidden=false;$('#equipmentSearch').focus();}});
  ...
  $('#sourceTable').addEventListener('click',event=>{const remove=event.target.closest('[data-delete-source]');if(remove){currentPoint().sources.splice(Number(remove.dataset.deleteSource),1);renderSources();renderCorrelations();markDraftDirty();return;}const select=event.target.closest('[data-equipment]');if(select){equipmentSourceIndex=Number(select.dataset.equipment);masterEquipmentRunIndex=null;$('#equipmentModal').hidden=false;}});
