(() => {
  ...
  function renderWorkflowActions() {
    ...
    let controls='';
    if(state.recordId&&['Draft','Reverted'].includes(state.status)&&canSave)controls='<button type="button" class="gum-btn" data-workflow="submit">Assign and Submit for Review</button><button type="button" class="gum-btn secondary" data-discard-draft>Discard Draft</button>';
  ...
  function renderContext() {
    ...
    const densityBasis=currentPoint().densityUnit==='m³/kg'?'specific volume: q = (m × v)/t':'density: q = m/(ρt)';
    modelInfo.dataset.info=coriolis?'Corrected master reference: qMM = qIndication × (1 + Correction). Relative error: E = ((MUT − qMM)/qMM) × 100. The selected master meter and its controlled correction are mandatory.':(state.quantity==='volume'?`Volumetric reference using ${densityBasis}. The base m³/s result is converted into the selected flow unit before relative error is evaluated.`:'Mass reference flow: q = m/t. The base kg/s result is converted into the selected mass-flow unit before relative error is evaluated. Density is excluded.');
    $('#formulaBox').textContent=coriolis?'Uses the corrected indication of the selected approved master meter as the reference for every observation.':(state.quantity==='volume'?'Uses retained mass, collection time and density to determine volumetric reference flow.':'Uses retained mass and collection time to determine mass reference flow.');
  ...
  function renderPoints() {
    ...
    $('#flowUnit').disabled=jobControlsPoints; $('#densityUnit').value=point.densityUnit||'kg/m³'; $('#densityUnitField').hidden=state.quantity!=='volume'||state.calculationType.endsWith('Cor');
  ...
  function renderRuns() {
    const coriolis=state.calculationType.endsWith('Cor');
    refreshObservationPreview();
    const headers=coriolis?['Run','Master meter','Master indication','Master correction','Correction basis','MUT indication','Reference flow','Error (%)','']:['Run','Collected mass (kg)','Collection time (s)','Density / specific volume','MUT indication','Reference flow','Error (%)',''];
    const rows=currentPoint().runs.map((run,index)=>{const evaluated=currentPoint().backendResult?.observations?.[index]||currentPoint().observationPreview?.observations?.[index]||evaluateRun(run)||run;return `<tr data-run="${index}"><td>${index+1}</td>${coriolis?`<td><button type="button" data-master-equipment="${index}">${esc(run.masterEquipmentLabel||'Select Equipment')}</button></td><td><input data-run-key="master" type="number" step="any" value="${esc(run.master)}"></td><td><input data-run-key="correction" type="number" step="any" value="${esc(run.correction)}"></td><td><select data-run-key="correctionBasis">${options([['','Select basis'],['curve','Correction curve'],['factor','Meter factor'],['certificate','Calibration certificate'],['k-factor','K-factor'],['equation','Approved correction equation']],run.correctionBasis)}</select></td><td><input data-run-key="mut" type="number" step="any" value="${esc(run.mut)}"></td>`:`<td><input data-run-key="mass" type="number" step="any" value="${esc(run.mass)}"></td><td><input data-run-key="time" type="number" step="any" value="${esc(run.time)}"></td><td><input data-run-key="density" type="number" step="any" value="${esc(run.density)}" ${state.quantity==='mass'?'disabled':''}></td><td><input data-run-key="mut" type="number" step="any" value="${esc(run.mut)}"></td>`}<td data-reference-output>${format(evaluated.reference_flow)}</td><td data-error-output>${format(evaluated.error_percent)}</td><td><button type="button" data-delete-run="${index}" aria-label="Delete observation">×</button></td></tr>`;}).join('');
  ...
  function evaluateRun(run) {
    ...
    if(state.calculationType.endsWith('Cor')){
      ...
      const reference=Number(run.master)*(1+Number(run.correction));
    ...
    if(state.quantity==='volume'&&(!valid(run.density)||Number(run.density)<=0))return null;
    let reference;
    if(state.quantity==='volume'){
  ...
  async function workflowAction(action,assignment=null){
    if(action==='submit'&&!assignment){await autosaveDraft();const actor=$('#gumApp').dataset.currentActor;$$('#technicalReviewer option,#finalApprover option').forEach(option=>{if(option.value)option.disabled=option.dataset.name===actor;});$('#reviewAssignmentModal').hidden=false;return;}
